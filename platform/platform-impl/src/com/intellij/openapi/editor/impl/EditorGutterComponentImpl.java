...
//    }
  ...
  private boolean shouldBeShown(@Nullable GutterMark gutterIconRenderer) {
    return false;//gutterIconRenderer != null && (areIconsShown() || gutterIconRenderer instanceof NonHideableIconGutterMark);
  }
...
//      }
  ...
  private boolean isHighlighterVisible(RangeHighlighter highlighter) {
    return false;//!FoldingUtil.isHighlighterFolded(myEditor, highlighter);
  }
...
//    }
  ...
  private int getFoldingAreaOffset() {
    return 0;//myLayout.getFoldingAreaOffset();
  }
  ...
  EditorMouseEventArea getEditorMouseAreaByOffset(int offset) {
    return null;//myLayout.getEditorMouseAreaByOffset(offset);
  }

  @Override
  public int getLineNumberAreaOffset() {
    return 0;//myLayout.getLineNumberAreaOffset();
  }

  @Override
  public int getAnnotationsAreaOffset() {
    return 0;//myLayout.getAnnotationsAreaOffset();
  }
  ...
  public int getLineMarkerAreaOffset() {
    return 0;//myLayout.getLineMarkerAreaOffset();
  }

  @Override
  public int getIconAreaOffset() {
    return 0;//myLayout.getIconAreaOffset();
  }
  ...
  public int getLineMarkerFreePaintersAreaOffset() {
    return 0;//myLayout.getLineMarkerFreePaintersAreaOffset();
  }

  @Override
  public int getExtraLineMarkerFreePaintersAreaOffset() {
    return 0;//myLayout.getExtraLeftFreePaintersAreaOffset();
  }
...
//    }
  ...
  private void updateHover(@Nullable GutterIconRenderer hoverGutterRenderer) {
    ...
    if (myCurrentHoveringGutterRenderer != null) {
      myEditorGutterListeners.getMulticaster().hoverEnded(new EditorGutterHoverEvent(this));
      myCurrentHoveringGutterRenderer = null;
    }
    if (hoverGutterRenderer != null) {
      myCurrentHoveringGutterRenderer = hoverGutterRenderer;
      myEditorGutterListeners.getMulticaster().hoverStarted(new EditorGutterHoverEvent(this));
    }
  }
