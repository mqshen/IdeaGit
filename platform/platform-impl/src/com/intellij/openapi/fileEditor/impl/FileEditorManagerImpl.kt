...
open class FileEditorManagerImpl(
  ...
  private fun doOpenFile(file: VirtualFile,
    ...
    if (ApplicationManager.getApplication().isWriteAccessAllowed) {
      ...
      return openFileImpl4(window = windowToOpenIn, _file = file, options = options)
    }
    else {
  ...
  open fun openFileImpl2(window: EditorWindow, file: VirtualFile, options: FileEditorOpenOptions): FileEditorComposite {
    ...
    return openFileImpl4(window = window, _file = file, options = options)
  }
   ...
   */
  internal fun openMaybeInvalidFile(window: EditorWindow, file: VirtualFile//, entry: HistoryEntry?
  ): FileEditorComposite {
    return openFileImpl4(window = window, _file = file//, entry = entry
...
//                             entry: HistoryEntry?,
    ...
    val effectiveOptions = getEffectiveOptions(options)
...
private fun getEffectiveOptions(options: FileEditorOpenOptions//, entry: HistoryEntry?
