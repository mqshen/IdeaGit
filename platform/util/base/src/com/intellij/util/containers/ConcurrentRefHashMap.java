...
abstract class ConcurrentRefHashMap<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V>, HashingStrategy<K> {
  ...
  private static final class RefEntry<K, V> implements Entry<K, V> {
  ...
  private final class EntrySet extends AbstractSet<Entry<K, V>> {
    private final Set<Entry<KeyReference<K>, V>> hashEntrySet = myMap.entrySet();

    @NotNull
    @Override
    public Iterator<Entry<K, V>> iterator() {
      return new Iterator<Entry<K, V>>() {
        private final Iterator<Entry<KeyReference<K>, V>> hashIterator = hashEntrySet.iterator();
        private RefEntry<K, V> next;

        @Override
        public boolean hasNext() {
          while (hashIterator.hasNext()) {
            Entry<KeyReference<K>, V> ent = hashIterator.next();
    ...
    public int hashCode() {
      int h = 0;
      for (Entry<KeyReference<K>, V> entry : hashEntrySet) {
